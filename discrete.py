...
def delta_params(x_input):
    ...
    print('graph scale: ', GRAPH_SCALE)
    if (shift == 0):
        SHIFT_FLAG = 0
    else:
        SHIFT_FLAG = 1

    print("type:::::::::::::::::::::typeeeeeeeee", type(GRAPH_SCALE))
    print(int(GRAPH_SCALE/2), "hereeeeeeee")
    delta[int(GRAPH_SCALE/2)-shift] = 1
    ...
    if(NEGATITVE_SCALE == 1):
        ...
        print("think pair share")
        plt.stem(np.arange(len(conv_output)), conv_output)
...
def unit_step_params():
    ...
    for i in range(GRAPH_SCALE):
        unit_step.append(0)
    shift = int(input("Enter the t0: "))
    for i in range(GRAPH_SCALE/2-shift, GRAPH_SCALE):
        unit_step[i] = 1
    ...
    plt.xlabel("n")
    plt.ylabel("x[n]")
    plt.stem(np.arange(-GRAPH_SCALE/2, GRAPH_SCALE/2, 1), unit_step)
    global_input = unit_step
    unit_step = unit_step[GRAPH_SCALE/2-shift:]
    return unit_step
    #plt.show()
